...
class ArithIntegerGenerator(FunctionGenerator):
    ...
    def generate_statement(self, opts, f, gen, literals, numbers):
        if opts['if'] > self.rng.random():
            ...
            exp1 = gen.generate_one_hot_encoded(list(literals) + numbers)
            exp2 = gen.generate_one_hot_encoded(list(literals) + numbers)
            ...
            exp = gen.generate_one_hot_encoded(list(literals) + numbers)
